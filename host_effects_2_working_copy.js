...
Data.then(function (data) {
  ...
  function createVisualisation () {
    ...
    var sumstat = getSumStat(data)
    // put at top or bottom of file
    ...
    d3.select('#Nodes2').on('click', function () {
      // make new data
      var sumstatN = getSumStat(data)
      console.log(sumstatN)
    ...
    d3.select('#Daystoflower2').on('click', function () {
      var filteredData = data.filter(function (d) {
        return d.Days_to_flower > 0;
      })
      var sum = getSumStat(filteredData)
      // update y scale domain
      y.domain([0, d3.max(data, function (d) {
        return d.maxDays + 0.1 * d.maxDays
      })])
